...
//SELECT DATE_ADD(pubdate, INTERVAL 3 DAY)

$sql="SELECT 	description,
				taskdescription,
				DATE_ADD(lastcompleted, INTERVAL frequencydays DAY) AS duedate,
				userresponsible,
				email
		...
		WHERE DATE_ADD(lastcompleted, INTERVAL (frequencydays-10) DAY) > NOW()
...
$sql="SELECT 	description,
		...
		INNER JOIN www_users
		ON fixedassettasks.userresponsible=www_users.userid
		WHERE DATE_ADD(lastcompleted, INTERVAL frequencydays DAY)> NOW()
		ORDER BY manager";
