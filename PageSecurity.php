...
$sql="SELECT script,pagesecurity,description FROM scripts";

$result=DB_query($sql, $db);

echo '<br /><form autocomplete="off"method="post" id="PageSecurity" action="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '">';
echo '<div class="container">';
echo '<input type="hidden" name="FormID" value="' . $_SESSION['FormID'] . '" />';

echo '<table class="table table-bordered">';

$TokenSql="SELECT tokenid, tokenname FROM securitytokens ORDER BY tokenid";
$TokenResult=DB_query($TokenSql, $db);

while ($myrow=DB_fetch_array($result)) {
           echo	'<tr><td>' . $myrow['description'] . '</td><td>' . $myrow['script'] . '</td><td><select name="' . $myrow['script'] . '">';

	while ($myTokenRow=DB_fetch_array($TokenResult)) {
		if ($myTokenRow['tokenid']==$myrow['pagesecurity']) {
			echo '<option selected="selected" value="' . $myTokenRow['tokenid'] . '">' . $myTokenRow['tokenid'] .'-'. $myTokenRow['tokenname'] . '</option>';
		} else {
			echo '<option value="'.$myTokenRow['tokenid'].'">' .  $myTokenRow['tokenid'] .'-'. $myTokenRow['tokenname'] . '</option>';
		}
	}
	echo '</select></td>
		</tr>';
