...
include_once 'includes/GoogleDriveApi.class.inc'; 
$statusMsg = ''; 
$status = 'danger'; 
    //if(isset($_GET['code'])){ 
    ...
    if(!empty($file_id)){ 
        ...
        $sqlQ = sprintf( "SELECT * FROM drive_files WHERE id = %s",$file_id); 
        $ResultIndex= DB_query( $sqlQ, $db);
        while ($fileData= DB_fetch_array($ResultIndex)) { 
            
            if(!empty($fileData['file_name'])){ 
                $file_name = $fileData['file_name']; 
                $target_file = 'uploads/'.$file_name; 
                $file_content = file_get_contents($target_file); 
                $mime_type = _mime_content_type($target_file); 
                    ...
                    try { 
                        ...
                        if($drive_file_id){ 
                            ...
                            if($drive_file_meta){ 
                                ...
                                $sqlQ =sprintf("UPDATE drive_files SET google_drive_file_id=%s WHERE id=%s",$db_drive_file_id, $db_file_id); 
                                DB_query($sqlQ,$db);
...
//} 
    ...
    var_dump($_SESSION['status_response']);
var_dump($_SESSION['last_file_id']);
