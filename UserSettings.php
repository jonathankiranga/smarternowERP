...
if (isset($_POST['Modify'])) {
	...
	if ($InputError != 1) {
	  ...
	  // update the session variables to reflect user changes on-the-fly
		$_SESSION['DefaultDisplayRecordsMax'] = $_POST['DisplayRecordsMax'];
...
If (!isset($_POST['DisplayRecordsMax']) OR $_POST['DisplayRecordsMax']=='') {
     $ResultIndex = DB_query("Select displayrecordsmax from www_users WHERE userid='". $_SESSION['UserID']."'",$db);
     $maxRow = DB_fetch_row($ResultIndex);  
     $_POST['DisplayRecordsMax'] = $maxRow[0];
     $_SESSION['DefaultDisplayRecordsMax'] = $maxRow[0];
}
